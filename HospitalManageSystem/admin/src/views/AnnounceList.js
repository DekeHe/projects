...
class AnnounceList extends Component {
    ...
    getAnnounceList(){
        ...
        axios.post(serverUrl+ '/announce/list',data)
            .then(function (response) {
        ...
        return (
            <div key={_this.props.location.key}  style={{margin: '20px 30px'}}>
                <span className="wrapper">
                  <Button type="success" onClick={ () => _this.setState({ dialogVisible: true, title: "添加公告信息", form: {..._this.state.form,id:""}}) }><i className="el-icon-plus"></i>添加</Button>
                  <Button type="warning" onClick={_this.openEdit.bind(_this)}><i className="el-icon-edit"></i> 修改</Button>
                  <Button type="danger" onClick={_this.deleteAnnounce.bind(_this)}><i className="el-icon-delete"></i>删除</Button>
                  <Input defaultValue={_this.state.form.searchContent} onChange={_this.changeInput.bind(_this, 'searchContent')} style={{width: '300px', marginLeft: '20px'}} placeholder="请输入公告内容" />
                ...
                >
                    <Dialog.Body>
                        <Form model={_this.state.form}>
                            <Form.Item label="公告内容" labelWidth="120">
                                <Input type="textarea" value={_this.state.form.content} onChange={_this.changeInput.bind(_this, 'content')} autosize={{ minRows: 3}} style={{width: '400px'}} placeholder="请输入公告内容"/>
                            </Form.Item>
                        </Form>
                    </Dialog.Body>
                    ...
                    <Dialog.Footer className="dialog-footer">
                        ...
                        <Button type="primary" onClick={_this.saveAnnounce.bind(_this)}>确 定</Button>
                    </Dialog.Footer>
                </Dialog>
            </div>
        );
    ...
    deleteAnnounce(){
        ...
        axios.post(serverUrl+ '/announce/delete', _this.state.form)
            .then(function (response) {
                let resp = response.data;
                if(resp.code === 0){
    ...
    saveAnnounce(){
        const _this = this;
        const {serverUrl } = _this.props;
        _this.state.form.userId = _this.state.user.id;
        // 保存操作
        axios.post(serverUrl+ '/announce/save',_this.state.form)
            .then(function (response) {
                let resp = response.data;
                if(resp.code === 0){
            ...
            errorOption.message = "网络错误，保存公告数据失败！";
            Message.error(errorOption);
        })
    }
    ...
    openEdit(){
        ...
        _this.setState({form: form}, () => {
            _this.setState({ dialogVisible: true, title: "编辑公告信息"});
        });
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(AnnounceList);
